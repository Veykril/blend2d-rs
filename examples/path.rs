...
fn main() {
    ...
    {
        let mut ctx = Context::new_from_image(&mut img).expect("Unable to attach rendering context");
        ctx.set_comp_op(CompOp::SrcCopy).unwrap();
        ctx.fill_all().unwrap();

        let mut path = Path::new();
        path.move_to(26.0, 31.0).unwrap();
        path.cubic_to(642.0, 132.0, 587.0, -136.0, 25.0, 464.0).unwrap();
        path.cubic_to(882.0, 404.0, 144.0, 267.0, 27.0, 31.0).unwrap();
